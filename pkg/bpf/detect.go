...
func LogFeatures() string {
	return fmt.Sprintf("override_return: %t, buildid: %t, kprobe_multi: %t, uprobe_multi %t, fmodret: %t, fmodret_syscall: %t, signal: %t, large: %t, link_pin: %t, missed_stats_kprobe_multi: %t, missed_stats_kprobe: %t",
		...
		HasLinkPin(), HasMissedStatsKprobeMulti(), HasMissedStatsPerfEvent())
}
