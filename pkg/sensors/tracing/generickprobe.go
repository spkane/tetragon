...
func createMultiKprobeSensor(multiIDs []idtable.EntryID, has hasMaps) ([]*program.Program, []*program.Map, error) {
...
func createGenericKprobeSensor(
	...
	if useMulti {
		progs, maps, err = createMultiKprobeSensor(ids, has)
	} else {
...
func createKprobeSensorFromEntry(kprobeEntry *genericKprobe,
	...
	if kprobeEntry.loadArgs.retprobe {
		loadret := program.Builder(
			...
			sensors.PathJoin(fmt.Sprintf("%s_return", kprobeEntry.funcName)),
			"generic_kprobe").
			SetRetProbe(true).
			SetLoaderData(kprobeEntry.tableId)
		progs = append(progs, loadret)
